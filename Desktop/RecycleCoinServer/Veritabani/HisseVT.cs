...
namespace RecycleCoinServer.Veritabani
{
    ...
    public class HisseVT
    {
        ...
        {
            var locHisse = AdresIleGetir(hisse.Adres);
            if (locHisse is null) HepsineEris().Insert(hisse);
            HepsineEris().Update(hisse);
        }
        ...
        {
            var hisses = HepsineEris();
            if (hisses is null) return null;

            hisses.EnsureIndex(_ => _.Adres);
            var hisse = hisses.FindOne(_ => _.Adres == adres);
            return hisse;
        }
